    ...
    <div class="row">
        <div class="col-md-6"><h3>Arabic Name : {{ $item->name_ar}}</h3>
            ...
            @foreach($extras as $extra)
                <h4>  {{ $extra->name }} <a href="/subitems/delete/{{$extra->id}}/{{ $item->id}}"
                                            class="btn btn-danger" type="button"
                                            onclick="return confirm('Are you sure you want to delete this OFFERS?')">X</a>
                </h4>
            @endforeach
            <hr/>
            <h2>Description Items</h2>
            @foreach($descriptions as $description)
                <h4>  {{ $description->name }} <a href="/desitem/delete/{{$description->id}}/{{ $item->id}}"
                                                  class="btn btn-danger" type="button"
                                                  onclick="return confirm('Are you sure you want to delete this OFFERS?')">X</a>
                </h4>
            @endforeach
            ...
            <a class="btn btn-primary" type="button" href="/desitem/create/{{$item->id}}">Add New Description</a>
        </div>
        <div class="col-md-6">
